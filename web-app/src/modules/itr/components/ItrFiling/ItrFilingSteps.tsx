...
import { ItrUploadRow } from './ItrUploadRow'
...
export const ItrStep2View = ({
  ...
  const panValidation = validateByPlaceholder(panNumber, 'PAN')
  const aadhaarValidation = validateByPlaceholder(aadhaarNumber, '12-digit Aadhaar number')
  const salaryValidation = validateByPlaceholder(salaryDetails, 'amount')
    ...
    <>
      ...
      <div className="itr-form-row-2">
        <div className="itr-input-group">
          ...
          <input
            type="text"
            className={`itr-text-input ${!panValidation.isValid && panNumber ? 'itr-text-input--error' : ''}`}
            placeholder="e.g. ABCDE1234F"
            value={panNumber}
            maxLength={10}
            onChange={(e) => setPanNumber(e.target.value.toUpperCase())}
          />
          {!panValidation.isValid && panNumber && (
            <span className="itr-field-error-text">⚠️ {panValidation.error}</span>
          )}
        </div>
        <div className="itr-input-group">
          ...
          <input
            type="text"
            className={`itr-text-input ${!aadhaarValidation.isValid && aadhaarNumber ? 'itr-text-input--error' : ''}`}
            placeholder="12-digit Aadhaar number"
            value={aadhaarNumber}
            maxLength={12}
            onChange={(e) => setAadhaarNumber(e.target.value)}
          />
          {!aadhaarValidation.isValid && aadhaarNumber && (
            <span className="itr-field-error-text">⚠️ {aadhaarValidation.error}</span>
          )}
        </div>
      </div>
      ...
      <div className="itr-upload-list">
        ...
        <div className="itr-input-group">
          ...
          <input
            type="text"
            className={`itr-text-input ${!salaryValidation.isValid && salaryDetails ? 'itr-text-input--error' : ''}`}
            placeholder="Enter amount or details"
            value={salaryDetails}
            onChange={(e) => setSalaryDetails(e.target.value)}
          />
          {!salaryValidation.isValid && salaryDetails && (
            <span className="itr-field-error-text">⚠️ {salaryValidation.error}</span>
          )}
        </div>
...
export const ItrStep3View = ({
  ...
  const v80C = validateByPlaceholder(deduction80C, 'amount')
  const v80D = validateByPlaceholder(deduction80D, 'amount')
  const vHomeLoan = validateByPlaceholder(homeLoanInterest, 'amount')
  const vEduLoan = validateByPlaceholder(eduLoanInterest, 'amount')
    ...
    <>
      ...
      <div className="itr-form-row-2">
        <div className="itr-input-group">
          ...
          <input
            type="text"
            className={`itr-text-input ${!v80C.isValid && deduction80C ? 'itr-text-input--error' : ''}`}
            placeholder="e.g. 1,50,000"
            value={deduction80C}
            onChange={(e) => setDeduction80C(e.target.value)}
          />
          {!v80C.isValid && deduction80C && (
            <span className="itr-field-error-text">⚠️ {v80C.error}</span>
          )}
        </div>
        <div className="itr-input-group">
          ...
          <input
            type="text"
            className={`itr-text-input ${!v80D.isValid && deduction80D ? 'itr-text-input--error' : ''}`}
            placeholder="e.g. 25,000"
            value={deduction80D}
            onChange={(e) => setDeduction80D(e.target.value)}
          />
          {!v80D.isValid && deduction80D && (
            <span className="itr-field-error-text">⚠️ {v80D.error}</span>
          )}
        </div>
      </div>
      ...
      <div className="itr-form-row-2">
        <div className="itr-input-group">
          ...
          <input
            type="text"
            className={`itr-text-input ${!vHomeLoan.isValid && homeLoanInterest ? 'itr-text-input--error' : ''}`}
            placeholder="e.g. 2,00,000"
            value={homeLoanInterest}
            onChange={(e) => setHomeLoanInterest(e.target.value)}
          />
          {!vHomeLoan.isValid && homeLoanInterest && (
            <span className="itr-field-error-text">⚠️ {vHomeLoan.error}</span>
          )}
        </div>
        <div className="itr-input-group">
          ...
          <input
            type="text"
            className={`itr-text-input ${!vEduLoan.isValid && eduLoanInterest ? 'itr-text-input--error' : ''}`}
            placeholder="e.g. 50,000 (Section 80E)"
            value={eduLoanInterest}
            onChange={(e) => setEduLoanInterest(e.target.value)}
          />
          {!vEduLoan.isValid && eduLoanInterest && (
            <span className="itr-field-error-text">⚠️ {vEduLoan.error}</span>
          )}
        </div>
      </div>
