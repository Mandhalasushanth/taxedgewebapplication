    ...
    <div className="compliance-field-content">
      <div className="compliance-field-label-group">
        <label className="compliance-field-label">
          {label}{" "}
          {required && <span className="compliance-required-mark">*</span>}
        </label>
        {badge}
      </div>
      {children}
  ...
  file ? (
    ...
    >
      ...
      <div className="compliance-upload-left">
        ...
        <div className="compliance-upload-meta">
          <span className="compliance-upload-title">
...
const ComplianceDocPreviewModal: React.FC<{
  file: File;
  title: string;
  onClose: () => void;
}> = ({ file, title, onClose }) => {
...
export const GSTComplianceCard: React.FC<GSTComplianceCardProps> = ({
  ...
  const [gstin, setGstin] = useState(initialGstin),
    [financialYear, setFinancialYear] = useState(initialFinancialYear);
  const [requestType, setRequestType] = useState<
    "Reconciliation Support" | "Notice Response"
  >(initialRequestType);
  const [purchaseFile, setPurchaseFile] = useState<File | null>(null),
    [salesFile, setSalesFile] = useState<File | null>(null);
  const [gstr2bRef, setGstr2bRef] = useState(""),
    [gstr2bFile, setGstr2bFile] = useState<File | null>(null);
  const [noticeNumber, setNoticeNumber] = useState(""),
    [noticeFile, setNoticeFile] = useState<File | null>(null);
  ...
  const validateForm = () => {
    ...
    if (!g) errs.gstin = "GSTIN or PAN is required";
      ...
      errs.gstin =
        "Invalid GSTIN format (e.g. 27AXTPD4419K1ZP with valid 10-char PAN)";

    if (!financialYear) errs.financialYear = "Financial Year is required";
    if (!requestType) errs.requestType = "Request Type is required";
    if (!purchaseFile)
      errs.purchaseFile = "Purchase register document is required";
    ...
    if (requestType === "Reconciliation Support") {
      ...
      if (!gstr2bRef.trim() && !gstr2bFile)
        errs.gstr2bRef =
          "GSTR-2B Reference number or statement file is required";
    } else {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      onSubmit?.({
        ...
        noticeNumber:
          requestType === "Notice Response" ? noticeNumber : undefined,
  ...
  if (isSubmitted) {
    return (
      <GSTComplianceSubmitted
        ...
        onBackToForm={() =>
          navigate(routePaths.gst.compliance, { replace: true })
        }
      ...
      <form onSubmit={handleSubmit} noValidate className="compliance-spec-form">
          ...
          badge={
            <span className="compliance-autofill-tag">
              <svg
                ...
                stroke="currentColor"
                strokeWidth="3"
                className="compliance-check-icon"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
        ...
        >
          <div className="compliance-select-wrapper">
            ...
            >
              {["FY 2026-27", "FY 2025-26", "FY 2024-25", "FY 2023-24"].map(
                (y) => (
        ...
        >
          <FileDropzone
            ...
            onFileView={(f) =>
              setPreviewDoc({ file: f, title: "Purchase Register" })
            }
          />
        </FieldRow>
        ...
        >
          <FileDropzone
            ...
            onFileView={(f) =>
              setPreviewDoc({ file: f, title: "Sales Register" })
            }
          />
        </FieldRow>
                ...
                >
                  ...
                  <div>
                    ...
                    <span className="compliance-attached-filesize">
                      {(gstr2bFile.size / (1024 * 1024)).toFixed(2)} MB ·
                      Statement attached
                    </span>
                  </div>
                </div>
